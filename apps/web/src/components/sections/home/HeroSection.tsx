      ...
      <Container>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 items-center gap-10 lg:gap-8">
          ...
          <div className="lg:col-span-6 flex flex-col items-start text-left z-10">
            {/* Pill Badge */}
            <Link href="/game" className="group inline-flex items-center gap-2 rounded-full border border-[#C59758]/40 dark:border-[#D5AE63]/40 bg-card/80 hover:bg-[#C59758]/10 px-4 py-2 text-sm font-semibold text-muted-foreground shadow-2xs backdrop-blur-xs mb-6 transition-all">
              <Sparkles className="h-4 w-4 text-accent-gold group-hover:scale-110 transition-transform" />
            ...
            <h1 className="font-serif text-4xl sm:text-5xl lg:text-[64px] font-normal leading-[1.1] tracking-tight text-foreground text-balance">
              ...
              Your <span className="text-accent-brand dark:text-accent-gold font-medium">Destiny.</span>
            </h1>
        ...
        <div className="mt-8 sm:mt-12 flex justify-start overflow-x-auto scrollbar-none -mx-4 px-4 sm:mx-0 sm:px-0">
          <div className="inline-flex items-center gap-4 sm:gap-6 rounded-full border border-border dark:border-[#D5AE63]/35 bg-card/80 backdrop-blur-xs px-5 sm:px-7 py-2.5 sm:py-3 shadow-2xs shrink-0">
              ...
              return (
                <div key={item.label} className="flex items-center gap-4 sm:gap-6">
                  <div className="flex items-center gap-2 sm:gap-2.5 text-xs sm:text-[15px] font-medium text-foreground/90">
                    <Icon className="h-4 w-4 sm:h-4.5 sm:w-4.5 text-accent-gold shrink-0" />
                    <span className="tracking-normal whitespace-nowrap">{item.label}</span>
                  </div>
                  {index < trustIndicators.length - 1 && (
                    <span className="inline-block h-3 sm:h-4 w-px bg-border dark:bg-[#D5AE63]/25" />
                  )}
                </div>
              );
            })}
